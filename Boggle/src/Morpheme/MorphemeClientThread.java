...
public class MorphemeClientThread extends Thread {
	...
	public void loadBoard(){
		...
		if (count == 2000){
			...
			MorphemeServer.jta.setText(randomWord + count);
			continue;
		}
		...
		for (int v = 0; v < wordsInBoard.size(); v++){
			if (!MorphemeServer.personalWords.containsKey(wordsInBoard.get(v))){
	...
	public int howManyWordsInArrayInFile(ArrayList<String> wordsInBoard, String type){
		...
		for (int f = 0; f < wordsInBoard.size(); f++){
			if (hashInQuestion.containsKey(wordsInBoard.get(f)) && !MorphemeServer.personalWords.containsKey(wordsInBoard.get(f))){
				number++;
			}
		}
